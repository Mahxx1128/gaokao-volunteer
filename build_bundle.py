"""Build single-file gaokao-volunteer.html bundle"""
import os
...
with open('js/data.js', 'r', encoding='utf-8') as f:
    data_js = f.read()
data_js = data_js.replace('window.GAOKAO_DATA = GAOKAO_DATA;', '// GAOKAO_DATA is already global (inline script)')

with open('js/predict.js', 'r', encoding='utf-8') as f:
    predict_js = f.read()

# App JS adapted for ECharts 4.x (built-in china map, no geoJSON loading needed)
app_js = r"""console.log('[高考志愿助手] v6 单文件版 · 文科503分 · 460~530分档');
...
// Wait for both DOM and map GeoJSON to be ready
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    tryAppInit();
    document.addEventListener('map-ready', tryAppInit);
  });
} else {
  tryAppInit();
  document.addEventListener('map-ready', tryAppInit);
...
fetch('https://geo.datav.aliyun.com/areas_v3/bound/100000_full.json')
  ...
  .then(function(geo) {
    ...
    console.log('[地图] 中国地图 GeoJSON 加载成功');
  })
  .catch(function(e) {
    console.error('[地图] 加载失败，请确认网络连接:', e);
  });
</script>
<style>
{css}
...
html = html_template.replace('{css}', css).replace('{data_js}', data_js).replace('{predict_js}', predict_js).replace('{app_js}', app_js)
